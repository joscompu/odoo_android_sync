...
class OdooAndroidSync(http.Controller):
        ...
        def get_attendance_hours(self, user_id, day_week, **kw):
                try:
                    calendar_id = http.request.env['hr.employee'].sudo().search([('user_id', '=', int(user_id))]).resourse_calendar_id.id
                    hours = http.request.env['resource.calendar.attendance'].sudo().search_read([('calendar_id', '=', int(calendar_id)), ('day_week', '=', )],
                                       ['hour_from', 'hour_to'])
